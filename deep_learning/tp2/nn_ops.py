...
def initNetwork(nn_arch, act_func_name):
    ...
    for i in range(np.size(nn_arch)-1):
        ...
        w = np.random.normal(0,1.0,nn_arch[i+1]*nn_arch[i]).reshape(nn_arch[i+1],nn_arch[i]) 
        W.append(w)
...
def backward(error,W,Yp):
    ...
    for i in range(len(W)-1,0,-1):
        err = np.transpose(W[i]).dot(gradB[len(W)-i-1])*(Yp[i-1])
        gradB.insert(0,err)
    return gradB
...
def update(eta, batch_size, W, B, gradB, Y):
    ...
    for i in range(len(W)):
        W[i] = updateParams(W[i],gradB[i].dot(Y[i].transpose())/batch_size,eta)
    for i in range(len(W)-1,0,-1):
        B[i] = updateParams(B[i],((gradB[i]).sum(axis=1)/batch_size).reshape(-1,1), eta)
    return W,B
